...
Field gaps say what is empty; they cannot say that the Finance Director appears under roles and authorization while no procedure step reaches that tier, or that nothing says what happens to a refused request. When no blocking gap remains, no document rule or conflict is unsettled, and the person has not said they are out of time, `runTurn.ts` makes one structured model call (`agent/consistencyReview.ts`, once per turn, only when the stated claims changed) that returns up to four findings. `sop-core/consistencyReview.ts` validates the output (ids must exist, the count and length limits hold, an earlier finding must be carried or resolved) and stores it as `session.consistencyReview`; the interview agenda then hands the agent one `consistencyQuestion` at a time (each once, at most four a session). It is deliberately not a gap: it is a question with no effect on `readyToReview`, `checkFinalization` or the PDF, it is not a claim (no tool, no status, `applyClaim` is untouched), and a failed or refused review is skipped (fail-open) and not retried until the claims change. The review reads only what the person stated, never the conversation, a suggestion or a document. A known, accepted limit: a model step that both streams a consistency question and calls tools can make one reply ask two, since only the last step's question is marked offered. Its output schema states no lengths or counts, because provider structured output does not reliably support them; code enforces them.
...
pnpm smoke:api     # live check of the five tools, one document extraction and one consistency review on both models; needs OPENAI_API_KEY

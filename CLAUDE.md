...
Implementation is in progress by vertical slices (see `docs/implementation-plan.md`). Slices 1 to 4 are built: the walking skeleton (chat, agent, readiness panel), the full interview (corrections, ordered procedure steps, an interview policy computed in code, a claims view, and an eval harness), review and approval (confirm and reject, an approval step, an SOP preview, and a locked session after approval), and PDF export (`POST /sops/pdf`, the download button, and a leave-page warning). Document upload is not. `spikes/extraction/` is the Phase 0 experiment, kept until slice 5 promotes its parsers.

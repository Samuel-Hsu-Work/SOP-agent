...
Implementation is in progress by vertical slices (see `docs/implementation-plan.md`). Slices 1 to 3 are built: the walking skeleton (chat, agent, readiness panel), the full interview (corrections, ordered procedure steps, an interview policy computed in code, a claims view, and an eval harness), and review and approval (confirm and reject, an approval step, an SOP preview, and a locked session after approval). PDF export and document upload are not. `spikes/extraction/` is the Phase 0 experiment, kept until slice 5 promotes its parsers.
...
Review, acknowledgement and approval run in the browser through `sop-core`; there is no approval route on the API. `applyClaim` and the acknowledgement function refuse every write on an approved session, and slice 4's PDF endpoint must call `checkFinalization` itself, because a forged session can still claim to be approved (the API guarantees the shape of an approval, not that a person clicked). The session document has a `schemaVersion` (now 3); a session stored by an older version is discarded with a notice, never migrated.
...
- `apps/web` (Next.js) owns the canonical session in `sessionStorage` (`lib/sessionStore.ts`) and replaces it only when a turn commits (`lib/chatTurn.ts`, `lib/useSopSession.ts`). `useSopSession` also applies review clicks, acknowledgements and approval locally (refused while a chat turn is in flight, built on the latest session from a ref). The review panel is built from `buildClaimsView` and the approval step from `buildApprovalView` (`lib/`, pure view models); each field expands to its claims, their earlier versions, and removed claims, and the SOP preview renders only `buildSopDocument`.
